...
extern int NDECL(dopurify); /**/ /* Medium */
...
*/
		pline("警告！探検モードに入ったら通常のゲームには戻れない。");
/*JP
...
*/
	else if (u.ualign.record >= -3)	you_are_ing("信仰の道を外れて");
/*JP
	else if (u.ualign.record >= -8)	you_have("sinned");
*/
	else if (u.ualign.record >= -8)	you_are_ing("神に背いて");
/*JP
...
*/
	if (Strangled) you_are_ing((u.uburied) ? "生き埋めにされて" : "窒息して");
	if (Glib) {
#if 0 /*JP:T*/
...
*/
	...
	if (Clairvoyant) you_have("周囲を透視する能力");
...
*/
	...
	if (Teleportation) you_can("テレポートが");
	if (Teleport_control) you_have("テレポートの制御能力");
	if (Lev_at_will) you_have("自分の意志で浮遊する能力");
	...
	if (Breathless) you_can("空気なしで生きることが");
...
*/
		enl_msg("あなたの悪運は時間がたっても良くならな", "い", "かった", "");
	    if (ltmp >= 0)
/*JP
		enl_msg("Good luck ", "does", "did", " not time out for you");
*/
		enl_msg("あなたの幸運は時間がたっても悪くならな", "い", "かった", "");
	}
...
*/
		...
		enl_msg(You_, "have not wished", "did not wish",
			" for any artifacts");
#else
		enl_msg("あなたはアーティファクトを願", "っていない", "わなかった", "");
#endif
	}
...
struct ext_func_tab extcmdlist[] = {
	...
	{"jump", "他の位置に跳躍する", dojump, FALSE},
	{"loot", "床の上の箱・袋を開ける", doloot, TRUE},
...
#endif
	{"rub", "ランプなどをこする", dorub, FALSE},
	{"sit", "座る", dosit, FALSE},
	{"turn", "アンデッドを祓う", doturn, TRUE},
	{"twoweapon", "二刀流のきりかえ", dotwoweapon, FALSE},
...
/*JP	pline("Your wielding weapon has no other use than a melee combat.");*/
	pline("あなたの装備している武器は接近戦専用だ.");
	return 0;
}
...
int rangelimit;
{
	...
	for (dx = 3; dx >= 0; dx-- ) {
		for (dy = dx; dy >= 0; dy-- ) {
			if ((dx*dx + dy*dy) > rangelimit) continue;
#endif
...
/*JP		You("don't wield any long range weapon.");*/
		You("長距離用の武器を装備していない.");
		return FALSE;
	}
	if (u.uswallow) {
	...
	if (ulasttgt) {		/* if last target is alive and in range, attack it */
		x = ulasttgt->mx;
		y = ulasttgt->my;
		mtmp = findtarget(x, y);
		if (mtmp && distu(x, y) <= weapon_range()) {
			(void) attack(mtmp);
	...
	if (mtmp = autotarget(weapon_range())) {
		(void) attack(mtmp);
...
#else
			You("手にした武器を勢いよく空中%s.",
			    objects[uwep->otyp].oc_dir ? "に突き出した" : "で振り回した");
#endif
			return TRUE;
		}
/*JP		You("don't see any target in range.");*/
		You("射程内にターゲットを見つけられない.");
		return FALSE;
	}
	return TRUE;
}
