	...
	switch (ptr->mlet) {
		...
		if(is_mercenary(ptr)) {
			...
			case PM_WATCH_CAPTAIN:
			  w1 = rn2(2) ? LONG_SWORD : IRON_SABER;
			  break;
			default:
			  if (!rn2(4)) w1 = DAGGER;
			  if (!rn2(7)) w2 = SPEAR;
		    ...
		    if (mm == PM_ELVENARCHER) {
		      (void)mongets(mtmp, ELVEN_BOW);
		      m_initthrow(mtmp, ELVEN_ARROW, 20);
		      break;
		    }
		    switch (rn2(3)) {
			case 0:
...
//			otmp = oname(otmp, artiname(
			    rn2(2) ? SILVER_LONG_SWORD : SILVER_SABER);
		    bless(otmp);
		...
		if (mm == PM_HOBBIT) {
			...
			(void)mongets(mtmp, !rn2(3) ? PICK_AXE : DAGGER);
                       ...
                       otmp = mksobj(OIL_LAMP, TRUE, FALSE);
                           (void) mpickobj(mtmp, otmp);
...
# endif
		...
		break;
	    case S_KOBOLD:
	      if (!rn2(mm == PM_LARGE_KOBOLD ? 3 : mm == PM_KOBOLD_LORD ? 6 : 12))
		    (void) mongets(mtmp, JAVELIN);
		else if (!rn2(4)) m_initthrow(mtmp, DART, 12);
		break;
	...
	*mtmp = zeromonst;		/* clear all entries in structure */
	...
	switch(ptr->mlet) {
		...
		case S_ORC:
			...
			break;
		case S_UNICORN:
		  /* horses always friendly to medium */
	...
	} else if (ptr == &mons[PM_MAID]) {
		(void)mongets(mtmp, MAID_DRESS);
		(void)mongets(mtmp, FRILLED_APRON);
		(void)mongets(mtmp, KATYUSHA);
		if(!rn2(4)) (void) mongets(mtmp, MIRROR);
		if(!rn2(6)) (void) mongets(mtmp, RIN_ADORNMENT);
		//if(!rn2(6)) (void) mongets(mtmp, LENSES);
