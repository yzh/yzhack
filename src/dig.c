...
*/
	const char *verb = (madeby == BY_YOU && uwep && is_axe(uwep)) ? "切り倒せない" : "掘れない";
...
#else
	    if (!Blind) pline(Hallucination ? "ママはどこだ！" :
 			"墓を荒してしまった！");
#endif
...
#else
	const char *verbing = ispick ? "掘る" : "切り倒す";
#endif
...
*/
		pline("%sとすると水の乱流が起きた。", jconj(verbing, "よう"));
	} else if(u.dz < 0) {
		if(Levitation)
/*JP
...
*/
		    pline("%sから岩のかけらを揺り落とした。", ceiling(u.ux, u.uy));
/*JP
