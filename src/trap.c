...
*/
glovecheck:		(void) rust_dmg(uarmg, "籠手", 1, TRUE, &youmonst);
...
*/
			    (void) rust_dmg(target, "籠手", 1, TRUE, mtmp);
			    break;
			case 2:
			    if (in_sight)
/*JP
...
struct trap *ttmp;
{
	...
	if ((mtmp = m_at(ttmp->tx,ttmp->ty)) != 0) {
		...
		You("%s%sを%sからはずした。", set_you[ttmp->madeby_u],
			(ttmp->ttyp == BEAR_TRAP) ? "トラバサミ" : "くもの巣",
			mon_nam(mtmp));
#endif
		reward_untrap(ttmp, mtmp);
	} else {
...
*/
			You("%sトラバサミを解除した。", set_you[ttmp->madeby_u]);
