...
*/
		    pline("%sを食べるのはよくないと感じた。", jtrns_mon(mons[pm].mname));
		    HAggravate_monster |= FROMOUTSIDE;
		}
		break;
	    case PM_LIZARD:
...
*/
		...
		if(!(HTeleportation & FROMOUTSIDE)) {
			...
			You_feel(Hallucination ? "diffuse." :
			    "very jumpy.");
#else
			pline(Hallucination ? "体が飛び散ったように感じた。" :
			    "跳躍力が高まったように感じた。");
#endif
