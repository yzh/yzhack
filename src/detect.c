...
*/
			Strcpy(buf, "あなたは物質的に恵まれていないと感じた。");
...
*/
    You("%sの間にいくらか金貨があることに気がついた。", body_part(FOOT));
    return(0);
...
*/
	    You("近くに%sがないことに気づいた。",what);
	    if (sobj && sobj->blessed) {
/*JP
		if (!u.uedibility) Your("%s starts to tingle.", body_part(NOSE));
*/
		if (!u.uedibility) Your("%sがうずきはじめた。", body_part(NOSE));
		u.uedibility = 1;
	    }
	} else if (sobj) {
	    char buf[BUFSZ];
...
#else
	    Sprintf(buf, "あなたの%sがひくつい%s。", body_part(NOSE),
			(sobj->blessed && !u.uedibility) ? "て、うずきだした" : "た");
#endif
...
*/
		if (!u.uedibility) pline("あなたの%sはうずきだした。", body_part(NOSE));
		u.uedibility = 1;
	}
    } else {
	struct obj *temp;
...
#else
	    	Your("%sはうずき%s、%sの匂いを嗅いだ。", body_part(NOSE),
...
*/
		Your("%sはうずき、%sの匂いを嗅いだ。", body_part(NOSE), what);
	}
/*JP
...
*/
    if (boulder && class != ROCK_CLASS) Strcat(stuff, "と巨石");
...
*/
		strange_feeling(detector, "あなたは何か足りないように感じた。");
	    return 1;
	}
...
*/
    You("%s気持になった。", sobj && sobj->cursed ? "とても貪欲な" : "罠にはまったような");
    display_nhwindow(WIN_MAP, TRUE);
    docrt();
    u.uinwater = uw;
    if (Underwater) under_water(2);
...
*/
	pline("気の毒に、%sを見ることができない。", the(xname(obj)));
	return;
    }
...
*/
	case 2 : pline("%sを覗いていると混乱した！", xname(obj));
...
*/
		pline("%sを覗いていると視力を奪われた！", xname(obj));
