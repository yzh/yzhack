...
*/
	if (i == 0) putstr(tmpwin, iflags.menu_headings, "アーティファクト");
	m = artidisco[i];
...
*/
	    ...
	    } else if (otmp->oartifact == ART_BUTTERFLY_KNIFE &&
		       (dieroll == 1 )) {
...
*/
		wepdesc = jtrns_obj('A', artilist[ART_BUTTERFLY_KNIFE].name);
		if (!youdefend) {
...
*/
				pline("%sは%sの%sを虚しく通り抜けた。", wepdesc,
...
*/
		if(!Blind) You("きらめく光球につつまれた！");
/*JP
		else You_feel("weightless for a moment.");
*/
		else You_feel("一瞬身体が浮かびあがるのを感じた。");
...
*/
	    if(on) You("暴徒を扇動しているように感じた。");
/*JP
	    else You_feel("the tension decrease around you.");
*/
	    else pline("まわりの張りつめた空気がゆるむのを感じた。");
	    break;
	case LEVITATION:
	    if(on) {
		float_up();
