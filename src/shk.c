...
*/
	static const char no_shk[] = "店は荒らされたようだ。";
...
*/
	    pline("%sはあなたの存在を感じとった。", shkname(shkp));
/*JP
	    verbalize("Invisible customers are not welcome!");
*/
	    verbalize("透明なお客様はご遠慮ください！");
	    return;
	}
...
*/
	    verbalize("ほう%s、%sの%sによくも戻ってこれたな！",
		      plname,
...
*/
		    pline("%sは満足し、突然に消えた！", shk_nam);
	} else if(wasmad)
/*JP
		pline("%s calms down.", Monnam(shkp));
*/
		pline("%sは怒りをおさめた。", Monnam(shkp));
...
*/
		    You("金がない%s。", stashed_gold ? "ようだ" : "");
		    if(stashed_gold)
/*JP
			pline("But you have some gold stashed away.");
*/
			pline("しかし、あなたは金をこっそり隠している。");
		} else {
#ifndef GOLDOBJ
		    long ugold = u.ugold;
		    if(ugold > ltmp) {
...
*/
			if (stashed_gold) pline("しかし、あなたは金を隠している！");
		    }
#ifndef GOLDOBJ
...
*/
		    pline("%sは金ではなく、血を求めている！", Monnam(shkp));
#ifndef GOLDOBJ
		    if(u.ugold < ltmp/2L ||
...
#else
		    You("%sの損失%sを肩代わりした。", mon_nam(shkp),
#endif
#ifndef GOLDOBJ
			  (u.ugold < ltmp) ? 
#else
...
#else
		    else Strcpy(sbuf,"拾った金や使った商品に対して、");
#endif
/*JP
		} else Strcat(sbuf, "for the use of merchandise.");
*/
		} else Strcpy(sbuf, "使った商品に対して、");
#if 1 /*JP*/
...
#else
		pline("拾った品物を買うには金が足りない。");
#endif
		if(stashed_gold)
/*JP
		    pline("Maybe you have some gold stashed away?");
*/
		    You("たぶん荷物の奥に金をしまいこんでいるのでは？");
		return(0);
	    }
...
*/
				"武器の見立て(NOT YET)", MENU_UNSELECTED);
	}
...
*/
		    Strcat(buf, (flags.female) ? "ご婦人" : "旦那");
	    }
	    if(ininv) {
		long quan = obj->quan;
...
#else
		pline("%s、%sは%sたったの金貨%ld枚です。」", buf, xname(obj),
			(quan > 1L) ? "1つ" : "", ltmp);
#endif
		obj->quan = quan;
	    } else
...
*/
	putstr(datawin, 0, "すでに使ってしまった未払いの品目：");
	putstr(datawin, 0, "");
...
#else
	    verbalize("注意してください%s、床を抜けて落ちてしまいますよ。",
...
*/
	if(Invis) pline("%sは不可視の魔力にだまされなかった！", shkname(shkp));
#if 0 /*JP*/
...
*/
		if (o == FOOD_CLASS) return "美食家も満足の";
...
#else
    "%sは言った：「ショッピングモールってのは頭痛のタネだ。」",
    "%sは言った：「落ち着いて、はっきり考えるんだ。」",
    "%sは言った：「ものごとは一度にひとつずつだ。」",
    "%sは言った：「インスタントコーヒーは好きじゃない…コロンビアン・スプレモをたのむ。」",
    "なんであれ開発チームの意見を一致させるのは困難だと%sは述べた。",
    "%sは神に尽くすやつが成功することに気づいた、と言った。",
    "%sは言った：「私から盗もうなんて思わないことだ。天上界に知り合いがいるからな。」",
    "%sは言った：「将来この店の品物が必要になることだろう。」",
    "%sは死の谷がゲートウェイとして作用していることについて言及した。"
#endif
};
...
#else
		pline("%sは最近どこかにほったらかしの店はないかとたずねた。",
#endif
		      Monnam(shkp));
...
#else
		pline("%sは%s客がどれほど嫌いかを語った。",
			shkname(shkp), 
			eshk->robbed ? "金を払わない" : "粗暴な");
#endif
...
#else
		pline("あなたは%sに%ld%sの借りがあると念を押した。",
		      shkname(shkp),
		      eshk->debit, currency(eshk->debit));
#endif
	else if (eshk->credit)
...
#else
		pline("%sは最近強盗にあったと愚痴をこぼした。", shkname(shkp));
#endif
#ifndef GOLDOBJ
...
*/
		verbalize("この手の品は扱っていないのですが、やってみましょう…");
	}
...
*/
		ident_type = yn_function("[b]基本サービス または [p]特別サービス",
		     ident_chars, '\0');
...
*/
		pline("%sは欲深にほくそ笑んだ…", mon_nam(shkp));
		mult = 1;
	...
	switch (obj->oclass) {        
		case AMULET_CLASS:      charge = 125 * mult;
					break;
		case WEAPON_CLASS:      charge = 25 * mult;
					break;
		case ARMOR_CLASS:       charge = 30 * mult;
					break;
		case FOOD_CLASS:        charge = 5 * mult;   
					break;
		case SCROLL_CLASS:      charge = 30 * mult;   
					break;
		case SPBOOK_CLASS:      charge = 50 * mult;   
					break;
		case POTION_CLASS:      charge = 30 * mult;   
					break;
		case RING_CLASS:        charge = 100 * mult;   
					break;
		case WAND_CLASS:        charge = 75 * mult;   
					break;
		case TOOL_CLASS:        charge = 25 * mult;   
					break;
		case GEM_CLASS:         charge = 200 * mult;
					break;
		default:                charge = 30 * mult;
					break;
	}
...
*/
		pline("%sが何か言ったがあなたは忘れてしまった。", mon_nam(shkp));
		return;
	    }
	}
...
*/
		pline("%sは忍び笑いをうかべて言った。「ほら、ばっちり呪いが解けた！」",
			mon_nam(shkp));
		obj->bknown = FALSE;
	}
	else if (Confusion)
...
*/
		    pline("血走った%sに気をとられた店主は、",
			makeplural(body_part(EYE)));
		    pline("間違って品物を祝福してしまった！");
		    bless(obj);
		}
		else
		{
...
*/
		verbalize("では%s。どんなものか見てみるとしましょうか。", slang);
		guesswork = FALSE;
	}
	else
	{
...
*/
		verbalize("なにしろこの分野の専門じゃないものでねぇ。");
		guesswork = TRUE;
	}
...
*/
		You("%sがこいつは相手をぶちのめすだろうと言うのを聞いた。",
			mon_nam(shkp));
		return;
	}
	//know_enchantment(obj);
	pline("Not supported yet!!");
}
...
*/
static const char we_offer[] = "私たちは最高のサービスを提供いたします！";
static void
shk_weapon_works(slang, shkp)
char *slang;
struct monst *shkp;
...
*/
	    verbalize("誰もあなたの%sに指一本さわれなくなりますよ！", xname(obj));
	    
	    /* Costs more the more eroded it is (oeroded 0-3 * 2) */
	    charge = 100 * (obj->oeroded + obj->oeroded2 + 1);
...
*/
	    verbalize("毒塗り%sがどれだけの代物か、想像がつくかい！", xname(obj));
...
*/
		     verbalize("あなたは鋼鉄の人と呼ばれるでしょうよ！");

		/* Costs more the more rusty it is (oeroded 0-3) */
		charge = 60 * (obj->oeroded+1);
/*JP
...
*/
			You("どうやって%sを着けるのかを忘れてしまった！", xname(obj));
		else if (Hallucination)
/*JP
			You("mistake your %s for a pot and...", xname(obj));
*/
			You("%sを鍋と間違えてしまい、ええと…", xname(obj));
	...
	struct monst *shkp;
{
	...
	char type = '\0';              /* Basic/premier service        */
...
*/
			type = yn_function("[b]基本サービス または [p]特別サービス",
					ident_chars, '\0');
			if (type == '\0') return;
...
*/
		pline("%sは充填は済んだと言って、あなたを扉のほうへ押しだした。",
			Monnam(shkp));
		return;
	}
...
*/
			verbalize("あなたは望む物がなんでも手に入るのですからね。");
			verbalize("…少しお金を融通してもらえますか？");
#ifndef GOLDOBJ
			shkp->mgold += u.ugold;
			u.ugold = 0;
#else
...
*/
	/* 日本語には複数のsは無い */
        Sprintf(sbuf, "%ldzorkmidかかります。よろしいですか？", charge);
...
*/
		verbalize("うちは現金払いだけだ、%s、あんたは金をもってない！",
			slang);
		return(FALSE);
	}
